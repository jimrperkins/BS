---
title: "Análisis de Grafos y Topología de Redes (II): Modularidad, Densidad, Longitudes Promedio de Camino, Assortivity"
---
...
3.  **Sumar las diferencias entre enlaces observados y esperados**: Para cada par de nodos $i$ y $j$, se calcula $A_{ij} - \frac{k_i k_j}{2m}$. Este término mide la diferencia entre el número real de enlaces entre los nodos de la misma comunidad y el número esperado si las conexiones fueran aleatorias.
...
El **diámetro de una red** es la longitud del camino más largo entre cualquier par de nodos en la red. Es decir, el diámetro es el valor máximo de las distancias más cortas $d(i,j)$ entre nodos $i$ y $j$. Esta medida es importante porque da una idea de cuán "extendida" o "compacta" es la red en términos de conectividad.
