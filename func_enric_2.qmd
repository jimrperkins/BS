...
Pathview genera un mapa visual de la vía, con los genes resaltados según su nivel de expresión y su posición en la vía. Esto permite ver claramente las regiones de la vía que están más afectadas por el conjunto de genes.
...
### Ejemplo del Cálculo de SPIA

En la práctica, puedes realizar SPIA como sigue:

```{r spia-ejemplo, warning=FALSE}
# Instalar y cargar el paquete SPIA
# install.packages("SPIA")  # Descomenta si no tienes SPIA instalado
library(SPIA)

# Datos de DEG de ejemplo (log fold changes de cada gen)
deg_data <- c(Gene1 = 2.5, Gene2 = -1.3, Gene3 = 1.2)

# ID de vía KEGG para una vía de señalización
id_via <- "hsa04010"  # Ejemplo: vía de señalización MAPK

# Realizar el análisis SPIA
spia_resultados <- spia(deg_data, pathway.id = id_via)
print(spia_resultados)
```

### Salida de SPIA

El resultado de SPIA incluye un puntaje de impacto para cada vía, indicando qué tan afectada está la vía por los cambios en la expresión génica. Las vías con un puntaje de impacto más alto están más afectadas por los cambios en la expresión génica.
